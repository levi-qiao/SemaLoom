...
| Releases | Authorized Studio save, compilation, revision history and tenant-scoped release activation in one PostgreSQL transaction | Independent candidate approval and online activation proofs are not implemented; general migration tooling and long-term audit retention remain open |
...
| Studio | Embedded graph/directory, inspector, structured canonical drafts, entity attribute/mapping editors (optional Metric vocabulary entries, no standalone Metric modeling center), source management, physical source trace, source connectivity validation, atomic save-and-activate, revision history, and deep links | Structured Rule editor is not available. The joint Studio gate (A64–A69) is not closed. Declared ONE same-source Links support collection group/filter by related attributes; many-to-many and cross-source JOIN are not open. No general OpenAPI discovery wizard, collaborative merge UI, or large-graph virtualization |
...
Review hardening: Rule compilation rejects incompatible units and output relabeling, invalid identity/grain,
undeclared or wrong-version package dependencies, and invalid Policy dates/dimensions. Missing/empty pack
paths fail explicitly. Each application request pins source destinations and records an environment binding
digest; prepared analysis must be prepared again after bindings change. This does not establish a source
snapshot, approval of an unvalidated source, production IAM, or the remaining T04 execution-profile gates.
Application adapters retain at most 64 destination pools per protocol until shutdown so a rebinding cannot
close an in-flight reader's pool; exhausting the limit fails reads and requires an application restart.
