...
`examples/financial-review` 包含 6 个对象、8 个关系、13 个指标、4 个规则（3 个判断和 1 个派生计算）。其中 `finance.ReviewCase` 固定到一份选定报告及其配对申报，含企业、年度、实际期间、配对策略、是否唯一候选、来源复核状态和抽取状态。SQL 视图属于独立 integration，核心没有财税表名或因子枚举。
...
原业务分析切片不依赖模型；后续 Chat 切片已按用户授权接通百炼并进行有界真实调用，尚不代表通用自然语言准确率评测。上述脚本和 HTTP 路径已实测；独立 AI 问答验收见 [测试提示词](analysis-test-goals.md)。

## 已验证与后续

主实现验证包含 typed Rule、非布尔 Claim 拒绝、标量身份真实存在性、非法/非有限数、搜索候选与租户边界、完整粒度、错误期间和证据引用。私有本地样本通过 80 次指标核对、30 次规则判断（16 TRUE、4 FALSE、10 UNKNOWN）、10 次期间拒绝；期望值直接由基础表独立计算，不调用被测规则生成答案。

本地样本保留在独立数据库；默认草稿在保留原定义的基础上新增分析定义。当前未执行发布审批，运行使用 local-dev 启动包 fallback；不把该结果称为已批准生产 release。现有不可变发布流程由原回归验证。
