...
从 [PLAN](docs/PLAN.md) 领取一个依赖已满足的任务，阅读 [AGENTS.md](AGENTS.md) 指向的规范。当前只有架构与 Studio 设计基线，尚无 Runtime 或 Python 工程；不要把预期命令报告为已通过。

## Implementation conventions

- 后端使用 Python + FastAPI，单个 `pyproject.toml`、`src/semaloom/` 包与 `uv.lock` 起步。T00 验证当前受维护的 Python 次版本与依赖组合，写入 `.python-version` 和 `requires-python`；不在架构阶段虚构已经测试过的兼容版本。
...
- 自动化检查采用 pytest、Ruff 和一种类型检查器，覆盖契约正反例、包依赖及集成行为；T00 建立实际命令与 CI。类型检查器由执行 agent 择一，不并行维护两套。
