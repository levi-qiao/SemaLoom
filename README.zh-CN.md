...
- 可选的 TypeSafe Jev 决策 hook，根据当前问题、有界上下文、本体目录和实时工具 Schema 做工具分类、本体候选选择与内部匹配评分；服务端仍负责参数校验、授权、查询和证据。
...
Studio Chat 的模型连接、Pi harness 安装和环境变量见 [Chat Harness](docs/chat-harness.md)。Jev 只接收当前问题、locale、脱敏且有界的会话摘要、已授权本体目录和通用工具 Schema；不会接收原始查询结果、数据库凭证、物理 Mapping 或业务身份值。它只在给定集合内建议下一步工具和语义候选，并用内部等级判断候选是否足够直接；该等级不会显示成回答置信率。明确问题和省略式追问先走确定性本体路径，Jev 不能提供业务事实、授权、执行查询或生成证据。默认 shadow 模式；外部 provider 缺失或故障时保留正常 Pi 路径。密钥只通过环境变量提供，不能提交到仓库。

采购示例现包含订单、组织、供应商和按字符串会计期间统计的合同，以及字典筛选、派生金额/比率、三值判断、同库/跨库关系和隔离合成数据；可运行问题见 [合成采购示例](examples/procurement/README.md)。
