...
from semaloom.runtime.studio_control import DraftSnapshot, StudioDraftService
...
class StudioReleaseService:
    ...
    def apply_live(
        ...
        with self.registry.transaction() as conn:
            snapshot = self.drafts.load(tenant, draft_id, connection=conn)
            return self._activate(conn, tenant, actor, environment, snapshot)
    ...
    ) -> tuple[DraftSnapshot, dict[str, Any]]:
        with self.registry.transaction() as conn:
            snapshot = self.drafts.save(
                tenant, actor, draft_id, documents, expected_revision, connection=conn
            )
            live = self._activate(conn, tenant, actor, environment, snapshot)
